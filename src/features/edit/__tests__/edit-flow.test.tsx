...
import { readSidecar, writeSidecar } from '~/lib/fs/clipSidecar';
import {
...
import { useGlobalShortcuts } from '~/features/shortcuts/shortcuts';

vi.mock('~/lib/fs/clipSidecar', () => ({
  SIDECAR_VERSION: 1,
  readSidecar: vi.fn(),
  writeSidecar: vi.fn(),
}));

vi.mock('~/features/preview/components/Preview', () => ({
  Preview: () => <div data-testid="preview-mock" />,
}));

vi.mock('~/features/timeline/components/Transport', () => ({
  Transport: () => <div data-testid="transport-mock" />,
}));

vi.mock('~/features/clips/components/ClipList', async () => {
  const { EditToggleButton } = await import('../components/EditToggleButton');
  return {
    ClipList: () => (
...
vi.mock('~/features/grade/components/Inspector', () => ({
  Inspector: () => <div data-testid="inspector-mock" />,
}));

vi.mock('~/features/deliver/components/ExportPanel', () => ({
  ExportPanel: () => <div data-testid="export-panel-mock" />,
}));

vi.mock('~/features/grade/components/LutSection', () => ({
  LutSection: () => <div data-testid="lut-section-mock" />,
}));

vi.mock('~/features/grade/components/ExposureSection', () => ({
  ExposureSection: () => <div data-testid="exposure-section-mock" />,
}));

vi.mock('~/features/grade/components/RenderModeSection', () => ({
  RenderModeSection: () => <div data-testid="render-mode-section-mock" />,
}));

vi.mock('~/features/grade/components/HdrSection', () => ({
