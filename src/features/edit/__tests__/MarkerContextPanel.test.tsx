...
import { MarkerContextPanel } from '../components/MarkerContextPanel';

vi.mock('~/features/clips/clipDataStore', async () => {
  const actual =
    await vi.importActual<typeof import('~/features/clips/clipDataStore')>(
      '~/features/clips/clipDataStore',
    );
  return actual;
});
...
describe('MarkerContextPanel', () => {
  ...
  it('renders nothing when the marker is missing', () => {
    useClipDataStore.setState({
      entries: {
        'clip-1': { markers: [], segments: [], baseGrade: {}, status: 'idle', readOnly: true },
      },
    });
