...
import type { SidecarV2 } from '~/lib/fs/clipSidecar';
import { readSidecar, SIDECAR_VERSION, writeSidecar } from '~/lib/fs/clipSidecar';
...
describe('clipDataStore.addSegment', () => {
  ...
  it('inserts a segment with default playMode/speed, returns its id, writes sidecar', async () => {
    ...
    await flush();
    expect(mockedWrite).toHaveBeenCalledTimes(1);
    const payload = mockedWrite.mock.calls[0]![2] as SidecarV2;
