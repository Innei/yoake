...
describe('formatTimeForFilename', () => {
  it('formats zero as 00-00-00-000', () => {
    expect(formatTimeForFilename(0)).toBe('00-00-00-000');
  });

  it('formats a time of 1h 2m 3.456s as 01-02-03-456', () => {
    expect(formatTimeForFilename(3723.456)).toBe('01-02-03-456');
  });

  it('rounds milliseconds to nearest ms', () => {
    expect(formatTimeForFilename(0.001)).toBe('00-00-00-001');
  });

  it('handles times beyond 24h by clipping hours to two digits', () => {
    expect(formatTimeForFilename(99 * 3600)).toBe('99-00-00-000');
  });
});

describe('extractFrame', () => {
  it('writes a PNG via getFileHandle + createWritable + write + close', async () => {
  ...
  it('closes the writable even if write throws', async () => {
    ...
    const handle = {
      async getFileHandle(): Promise<MockFileHandle> {
        return {
          createWritable: vi.fn(async (): Promise<MockWritable> => ({
            write: vi.fn(async () => {
