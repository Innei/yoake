...
async function pickLutDirectory(): Promise<FileSystemDirectoryHandle | undefined> {
...
export function LutSection() {
  ...
  const currentLutLabel = lutDescriptor
    ? lutDescriptor.name.replace(/\.cube$/i, '')
    : undefined;
            ...
            {popoverOpen ? (
              ...
              >
                {luts.length === 0 ? (
                  ...
                  luts.map((entry) => {
                    const selected = lutDescriptor?.id === entry.id;
                    return (
                      <button
