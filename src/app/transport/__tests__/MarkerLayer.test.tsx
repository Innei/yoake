...
describe('MarkerLayer', () => {
  it('renders one dot per marker positioned by time', () => {
    seedClip([
      { id: 'm1', time: 2, label: '' },
      { id: 'm2', time: 5, label: 'hero' },
    ]);
    const { getByTestId } = render(<MarkerLayer />);
    const dot1 = getByTestId('transport-marker-dot-m1') as HTMLElement;
    const dot2 = getByTestId('transport-marker-dot-m2') as HTMLElement;
    expect(dot1.style.left).toBe('20%');
    expect(dot2.style.left).toBe('50%');
  });

  it('right-click on marker opens 2-item menu', async () => {
    seedClip([{ id: 'm1', time: 2, label: '' }]);
    const { getByTestId } = render(<MarkerLayer />);
    const dot = getByTestId('transport-marker-dot-m1');

    fireEvent.contextMenu(dot, { clientX: 50 });
  ...
  it('view mode renders a non-interactive dot with no menu', () => {
    seedClip([{ id: 'm1', time: 2, label: '' }]);
    useEditModeStore.setState({ mode: 'view' });

    const { getByTestId } = render(<MarkerLayer readOnly />);
    const dot = getByTestId('transport-marker-dot-m1');
    fireEvent.contextMenu(dot, { clientX: 50 });
