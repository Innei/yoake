...
import fragSource from '../shaders/rawDlogPreview.frag.wgsl?raw';

export interface RawDlogPreviewPipeline {
  destroy: () => void;
  run: (
...
export function createRawDlogPreviewPipeline(device: GPUDevice): RawDlogPreviewPipeline {
  ...
  return {
    run(externalTexture, targetView, encoder) {
      if (destroyed) {
        throw new Error('rawDlogPreview pipeline has been destroyed');
      }
